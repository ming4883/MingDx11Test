...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE ignoreMe0, LPSTR ignoreMe1, INT ignoreMe2)
{
    ...
    if (xprAppContext.multiSampling)
    {
        int pixelAttribs[] =
        {
            WGL_SAMPLES_ARB, 16,
	...
	sprintf(glVersionStr, "GL_VERSION_%d_%d", xprAppContext.apiMajorVer, xprAppContext.apiMinorVer);
    if ((xprAppContext.apiMajorVer > 2) && glewIsSupported(glVersionStr))
    {
        const int contextAttribs[] =
        {
            WGL_CONTEXT_MAJOR_VERSION_ARB, xprAppContext.apiMajorVer,
            WGL_CONTEXT_MINOR_VERSION_ARB, xprAppContext.apiMinorVer,
